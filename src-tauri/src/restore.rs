...
pub fn do_restore(save_name: String) -> Result<(), String> {
 ...
 */
    ...
    let restore_dest = PathBuf::from(&default_saves_path).join(&original_save_name);
    dircpy::copy_dir(&backup_root, &restore_dest).map_err(|e| e.to_string())?;
    Ok(())
}
