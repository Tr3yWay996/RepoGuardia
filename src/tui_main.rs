...
enum CurrentScreen {
    ...
    ChooseVersion // Option 4: Choose backup version
}
...
fn load_save_folders(app: &mut App) {
    ...
    if default_saves_path.is_empty() {
        app.items.push("Error: default_saves_path not set in config".to_string());
        return;
    }
...
fn load_backups(app: &mut App) {
    app.items.clear();
    app.status_message = "List of Backups (Press Enter to Restore if in Restore Mode, else its read only)".to_string();

    let destination_base_path = get_config_value("destination_base_path");
    if destination_base_path.is_empty() {
        app.items.push("Error: destination_base_path not set in config".to_string());
        return;
    }
        ...
        if let Ok(entry) = entry {
            if entry.file_type().is_dir() {
                ...
                if metadata_check.exists() {
                    ...
                    let display_name = if let Ok(contents) = std::fs::read_to_string(&metadata_check) {
...
fn perform_backup(app: &mut App, selected_index: usize) {
    ...
    if selected_index < dirs.len() {
        ...
        let version = if last_version.is_empty() { "0.0.0".to_string() } else { last_version };
        ...
        match dircpy::copy_dir(selected_path, &full_destination) {
            Ok(_) => {
                ...
                app.status_message = format!("Success! Backed up to {}", full_destination.display());
            },
            Err(e) => {
    ...
    if selected_index < backup_dirs.len() {
        ...
        if let Ok(metadata_contents) = std::fs::read_to_string(&metadata_path) {
            if let Ok(metadata) = serde_json::from_str::<serde_json::Value>(&metadata_contents) {
                let original_name = metadata.get("original_name").and_then(|v| v.as_str()).unwrap_or("");
                ...
                match dircpy::copy_dir(&save_folder, &full_destination) {
                    Ok(_) => app.status_message = format!("Restored {} successfully!", original_name),
