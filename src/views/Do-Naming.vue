...
<template>
    <div class="view-layout">
        <h1>Set backup label:</h1>
        <ul>
            <li v-for="([path, date, label], index) in saves" :key="path" @click="selectItem(index)":class="{ selected: index === selectedIndex }">
                {{ index + 1 }}: {{ formatName(path) }}
                <span v-if="label" class="label"> [{{ label }}]</span>   <span class="date">last modified: {{ date }}</span>
            </li>
        </ul>
